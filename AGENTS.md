...
```text
manifest.json   扩展配置
background.js   管理各标签页图标与 Popup、捕获 Authorization、校验 Popup 消息、转发任务
content.js      在网页上下文请求兑换接口
popup.html/js   Popup 界面、页面适用性提示与交互
icons/          同轮廓的蓝色/灰色扩展图标（16/48/128）
...
- `https://lingsuan.top/*` 页面使用蓝色图标，点击展示完整兑换界面；其他页面使用同轮廓灰色图标，点击仅提示“本插件需要在 lingsuan.top 页面中使用。”
- 非 `https://lingsuan.top/*` 页面不得展示 Authorization 状态、刷新按钮、兑换表单或兑换结果。
- 蓝色与灰色图标必须保持相同轮廓，并保留 16、48、128 三种尺寸。
